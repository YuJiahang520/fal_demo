...
/* 定义模拟 Flash 的总大小：1MB */
#define FLASH_FILE_NAME  "../virtual_flash1.bin"
...
static int pc_flash_init(void)
{
    ...
    if (fp == NULL)
    {
        ...
        if (fp == NULL)
        {
            printf("[Driver1] Error: Cannot create flash file.\n");
            return -1;
        }
        ...
        fflush(fp);
        printf("[Driver1] Created virtual flash file (%d bytes)\n", FLASH_TOTAL_SIZE);
    }
    else
    {
        printf("[Driver1] Opened existing virtual flash file.\n");
    }
...
static int pc_flash_read(long offset, uint8_t *buf, size_t size)
{
    ...
    printf("[Driver1] Read offset: %ld, size: %zu\n", offset, size);
...
static int pc_flash_write(long offset, const uint8_t *buf, size_t size)
{
    ...
    printf("[Driver1] Write offset: %ld, size: %zu\n", offset, size);
    return write_len;
}
...
static int pc_flash_erase(long offset, size_t size)
{
    ...
    printf("[Driver1] Erased offset: %ld, size: %zu\n", offset, size);
    return size;
}

/* 定义设备对象 */
const struct fal_flash_dev pc_nor_flash1 = {
    .name = "pc_nor_flash1",
    .addr = 0,
